...
const ExclusiveBenefits = () => {
  ...
  return (
    <div className="bg-yellow-50 border-4 border-pink-500 rounded-3xl p-6 text-center shadow-lg max-w-screen-lg mx-auto">
      <div className="flex flex-col items-center justify-center">
        
        {/* Tiêu đề */}
        <div className="relative bg-gradient-to-b from-yellow-300 to-yellow-500 text-red-600 
                        px-4 py-4 md:px-10 md:py-6 rounded-full shadow-lg max-w-3xl text-center 
                        border-4 border-yellow-500">
          <h2 className="text-2xl sm:text-3xl md:text-4xl font-extrabold drop-shadow-lg">
            9 Đặc Quyền Mua Hàng
          </h2>
          <p className="text-lg sm:text-xl md:text-2xl font-bold text-red-700 mt-2 drop-shadow-lg">
            Tại Bạch Long Mobile
          </p>
        </div>

        {/* Danh sách đặc quyền */}
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 mt-6 w-full px-4">
          {benefits.map((item, index) => (
            <a
              key={index}
              href={item.url}
              className="bg-orange-500 text-white py-3 px-4 rounded-lg shadow-md flex 
                         items-center space-x-3 transition transform hover:scale-105 
